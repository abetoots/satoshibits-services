...
import { pipe, flow, identity, constant, compose, tap } from './composition.mjs';
...
describe('identity', () => {
  ...
  it('is useful as a default transform', () => {
    const transform = (fn: (x: number) => number = identity) => (x: number) => fn(x);
...
describe('constant', () => {
  ...
  it('ignores any arguments passed', () => {
    ...
    expect((alwaysHello as (n1: number, n2: number, n3: number) => string)(1, 2, 3)).toBe('hello');
  });
...
describe('tap', () => {
  ...
  it('preserves value even if side effect throws', () => {
    ...
    expect(() => pipe(42, tap(dangerousSideEffect))).toThrow('Side effect error');
  });
...
describe('Type inference', () => {
  ...
  it('handles union types', () => {
    const process = (x: string | number) => pipe(
      x,
