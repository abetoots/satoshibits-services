...
export function flow<Args extends unknown[], B>(fn1: (...args: Args) => B): (...args: Args) => B;
export function flow<Args extends unknown[], B, C>(fn1: (...args: Args) => B, fn2: (b: B) => C): (...args: Args) => C;
export function flow<Args extends unknown[], B, C, D>(fn1: (...args: Args) => B, fn2: (b: B) => C, fn3: (c: C) => D): (...args: Args) => D;
...
export function compose<Args extends unknown[], B>(fn1: (...args: Args) => B): (...args: Args) => B;
export function compose<Args extends unknown[], B, C>(fn2: (b: B) => C, fn1: (...args: Args) => B): (...args: Args) => C;
export function compose<Args extends unknown[], B, C, D>(fn3: (c: C) => D, fn2: (b: B) => C, fn1: (...args: Args) => B): (...args: Args) => D;
...
export function composeAsync<T>(...fns: ((arg: T) => Promise<T>)[]) {
  if (fns.length === 0) {
    return (x: T) => Promise.resolve(x);
  }
  if (fns.length === 1) {
    return fns[0];
  }
  ...
  return (initial: T) => {
    ...
    return fns.reduceRight<Promise<T>>(
...
export function pipeAsync<T>(...fns: ((arg: T) => Promise<T>)[]) {
  if (fns.length === 0) {
    return (x: T) => Promise.resolve(x);
  }
  if (fns.length === 1) {
 ...
 * @since 2025-07-03
 */
export const sequence = <T,>(fns: (() => Promise<T>)[]): Promise<T[]> =>
  fns.reduce(
